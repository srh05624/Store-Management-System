...
def create_backup(backup_path):
    try:
        if db is None:
            utils.log_error("Database not initialized. Cannot create backup.")
            return

        with sqlite3.connect(backup_path) as backup_conn:
            db._get_connection().backup(backup_conn)
        
        utils.log_info(f"Database backed up successfully to {backup_path}")
    except Exception as e:
        utils.log_error(f" > Error creating database backup: {str(e)}")

def restore_backup(backup_path):
    try:
        if db is None:
            utils.log_error("Database not initialized. Cannot restore backup.")
            return

        with sqlite3.connect(backup_path) as conn:
            result = conn.execute("PRAGMA integrity_check").fetchone()[0]
            if result != "ok":
                utils.log_error("Selected backup failed integrity check.")
                return
            
            conn.backup(db._get_connection())
        
        utils.log_info(f"Database restored successfully from {backup_path}")
    except Exception as e:
        utils.log_error(f" > Error restoring database backup: {str(e)}")

def get_database_path():
    global db
    if db is None:
        utils.log_error("Database not initialized. Cannot get database path.")
        return None
    return db.db_path
...
def initialize_database(db_path):
    try:
        ...
        db._get_connection().execute("PRAGMA foreign_keys = ON;")
        ...
        db.create_table('images', '''
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            history_id INTEGER NOT NULL REFERENCES service_history(id) ON DELETE CASCADE,
            file BLOB,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ''')
