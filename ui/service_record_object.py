...
class ServiceRecordWidget(QWidget):
    def __init__(
        ...
        if self.service_textbox:
            self.service_textbox.installEventFilter(self)
        ...
        if self.price_textbox:
            self.price_textbox.installEventFilter(self)
        ...
        if self.frequency_textbox:
            self.frequency_textbox.installEventFilter(self)
        ...
        if self.start_date_textbox:
            self.start_date_textbox.installEventFilter(self)
        ...
        if self.end_date_textbox:
            self.end_date_textbox.installEventFilter(self)
    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.FocusOut:
            if obj in [self.notes_textbox, self.service_textbox, self.frequency_textbox, self.start_date_textbox, self.end_date_textbox]:
                self.update_record()
            elif obj == self.price_textbox:
                self.update_price()
                self.update_record()
    ...
    def delete_record(self):
        if self.store_id and self.record_id:
            ...
            utils.log_debug(f"Deleted service record ID {self.record_id} for store ID {self.store_id}")
            if self.refresh:
                self.refresh()
        else:
    ...
    def update_price(self):
        if self.price_textbox:
            try:
                price_value = float(self.price_textbox.text().replace("$", ""))
                self.price = price_value
                self.price_textbox.setText(f"${price_value:.2f}")
            except ValueError:
            ...
            if self.service_textbox:
                ...
                if self.service_id and service and service[0]["id"] == int(self.service_id):
                    ...
                    new_service_id = database.add_service({
                        ...
                        "default_price": self.price_textbox.text() if self.price_textbox and self.price_textbox.text() else "0.00",
                        "description": "",
                    })
                    if new_service_id:
                        updated_data = {
